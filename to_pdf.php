...
//Date de naissance
$pdf->MultiCell(60,2, utf8_decode('né le').' '.$_POST['jour']." ".utf8_decode($_POST['mois'])." ".$_POST['annee']);
//coordonnées
if($_POST['telephone']){
$pdf->MultiCell(40,10,$_POST['telephone']);
}
if($_POST['email']){
$pdf->MultiCell(40,10,$_POST['email']);
}
if($_POST['site']){
	foreach($_POST['site'] as $cnt => $site) {
		$pdf->MultiCell(40,10,$_POST['site'][$cnt]);
	}
}
...
//OBJECTIFS
$pdf->MultiCell(40,10,"Formations");

$pdf->SetFont('Le-patin-helvete','',11);

if($_POST['annee-formation']){
	foreach($_POST['annee-formation'] as $cnt => $annee) {
		$pdf->MultiCell(40,10,$_POST['annee-formation'][$cnt]);
	}
	foreach($_POST['formation'] as $cnt => $formation) {
		$pdf->MultiCell(40,10,$_POST['formation'][$cnt]);
	}
}
